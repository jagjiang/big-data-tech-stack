...
trait HBaseUtils {
  ...
  class Admin(connection: Connection) {
     ...
     */
    def tableExist(tableName: String, family: String)(implicit wq: Writes[String]): Boolean = {
     ...
     * @return true if table exists, false otherwise
     */
    def tableExist(tableName: String, families: Set[String])(implicit wq: Writes[String]): Boolean = {
      val admin = connection.getAdmin
      val table = TableName.valueOf(tableName)
      if(admin.tableExists(table)){
